...
const BestiesScreen: React.FC<BestiesScreenProps> = ({ besties, setBesties }) => {
    ...
    const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            const fileName = `bestie_${Date.now()}_${file.name}`;
            try {
                const puterFile = await window.puter.fs.write(fileName, file);
                setNewBestie({ ...newBestie, image: puterFile.path });
            } catch (error) { console.error("Error uploading bestie image:", error); }
        }
    };
                    ...
                    {besties.map(b => (
                        <div key={b.id} className="bg-white dark:bg-[#3E3561] rounded-xl shadow-md p-4 flex items-center space-x-4 relative">
                            <PuterImage 
                                ...
                                alt={b.name}
                                fallback={<div className="w-20 h-20 rounded-full bg-[#E0B1CB] dark:bg-[#5E548E] flex items-center justify-center text-white"><Users size={40} /></div>}
                            />
                            ...
                            <button onClick={() => handleDelete(b.id)} className="absolute top-2 right-2 text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={18} /></button>
                        </div>
                    ))}
                </div>
            ) : (
                 <div className="flex flex-col items-center justify-center h-full text-center p-8 -mt-16 text-[#9A86A8] dark:text-gray-400">
                    <div className="text-[#BE95C4] dark:text-[#9F86C0] mb-4"><Users size={64} strokeWidth={1.5} /></div>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Añadir Bestie">
                <div className="space-y-4">
                    ...
                    <label className="w-full flex items-center justify-center p-4 border-2 border-dashed border-[#E0B1CB] dark:border-[#5E548E] rounded-lg cursor-pointer hover:bg-[#E0B1CB]/20 dark:hover:bg-[#5E548E]/40">
                        {newBestie.image ? (<PuterImage puterPath={newBestie.image} className="w-24 h-24 object-cover rounded-lg"/>) : (<div className="text-center text-[#9A86A8] dark:text-gray-400"><ImageIcon className="mx-auto" /><span>Añadir foto</span></div>)}
